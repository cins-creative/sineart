...
import { fetchMetaInsightsReport, type MetaInsightsReport } from "@/lib/data/meta-marketing-insights";
...
export async function GET(req: Request) {
  ...
  const res = await fetchMetaInsightsReport({ preset, customFrom, customTo });
  ...
  return NextResponse.json({ ok: true, ...(res.data satisfies MetaInsightsReport) });
}
