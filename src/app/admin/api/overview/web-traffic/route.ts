...
import { fetchWebTrafficReport, type WebTrafficReport } from "@/lib/data/web-traffic-ga4";
...
export async function GET(req: Request) {
  ...
  const res = await fetchWebTrafficReport({ preset, customFrom, customTo });
  ...
  return NextResponse.json({ ok: true, ...(res.data satisfies WebTrafficReport) });
}
