  ...
  return (
    <Card>
      ...
      <ol className="list-decimal space-y-2 pl-4 text-[12px] text-black/65">
        ...
        <li>Business Manager → System Users → Generate token với <code className="rounded bg-black/[0.04] px-1">read_insights</code>,{" "}
          <code className="rounded bg-black/[0.04] px-1">pages_read_engagement</code>,{" "}
          <code className="rounded bg-black/[0.04] px-1">ads_read</code>.
        </li>
...
export default function MetaInsightsCharts({
  ...
  const compareBarData = useMemo(() => {
    ...
    return [
      ...
      { metric: "Click Ads", current: cur.adClicks, previous: prev.adClicks },
    ];
  }, [payload]);
    ...
    <div className="flex flex-col gap-4 pb-2">
      <div>
        ...
        <p className="mt-1 max-w-2xl text-[12px] leading-snug text-black/55">
          Dữ liệu tự động từ Meta Graph API. Mỗi KPI so sánh với <strong className="font-semibold text-black/70">kỳ trước cùng độ dài</strong> — dùng để đối chiếu hiệu quả Fanpage/Ads thay nhập tay.
        </p>
      </div>
      ...
      {payload ? (
        <>
          <div className="flex flex-wrap gap-2.5">
            <CompareKpi label="Lượt xem Fanpage" value={payload.totals.pageMediaViews} delta={payload.deltas.pageMediaViews} color={C.blue} />
            <CompareKpi label="Tương tác nội dung" value={payload.totals.pageEngagements} delta={payload.deltas.pageEngagements} color={C.green} />
            <CompareKpi label="Theo dõi mới (ngày)" value={payload.totals.pageFollows} delta={payload.deltas.pageFollows} color={C.blue} />
            {payload.hasAdAccount ? (
              <>
                <CompareKpi label="Chi phí Ads" value={payload.totals.adSpend} delta={payload.deltas.adSpend} color={C.pink} format="money" />
                <CompareKpi label="Tin nhắn Ads" value={payload.totals.adMessages} delta={payload.deltas.adMessages} color={C.orange} />
                <CompareKpi label="Click link Ads" value={payload.totals.adClicks} delta={payload.deltas.adClicks} color={C.pink} />
              </>
            ) : (
              <div className="flex min-w-[200px] flex-1 items-center rounded-2xl border border-dashed border-[#EDE8E9] px-4 py-3 text-[11px] text-black/45">
                Chưa cấu hình <code className="mx-1 rounded bg-black/[0.04] px-1">META_AD_ACCOUNT_ID</code> — chỉ hiện Fanpage.
              </div>
            )}
          </div>
                  ...
                  <div className="h-[220px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <ComposedChart data={chartData} margin={{ top: 6, right: 8, left: 0, bottom: 4 }}>
                        ...
                        <Line yAxisId="right" type="monotone" dataKey="adClicks" name="Click" stroke={C.blue} strokeWidth={2} strokeDasharray="4 2" dot={false} />
                        <Legend wrapperStyle={{ fontSize: 11 }} />
                      </ComposedChart>
                    </ResponsiveContainer>
                  </div>
