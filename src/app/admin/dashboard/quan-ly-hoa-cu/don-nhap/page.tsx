...
import { fetchDonNhapPage, fetchHoaCuStaffStudentContext, HOA_CU_NHAP_PATH } from "@/lib/data/admin-hoa-cu";
...
export default async function QuanLyHoaCuDonNhapPage({ searchParams }: Props) {
  ...
  if (chiNhanhId !== resolvedChiNhanhId) {
    ...
    redirect(`${HOA_CU_NHAP_PATH}?${p.toString()}`);
  }
  ...
  const don = await fetchDonNhapPage(supabase, ctx.data, {
    page,
