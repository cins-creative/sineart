...
export default async function QuanLyHoaCuPage() {
  ...
  const bundle = await fetchAdminHoaCuBundle(supabase);
  if (!bundle.ok) {
    return (
      <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-sm text-red-800">
  ...
  return <QuanLyHoaCuView bundle={bundle.data} defaultStaffId={session.staffId} />;
}
