...
/** Chụp DOM phiếu lương → PNG → clipboard (Zalo/Messenger: dán ảnh). */
async function copyDomAsPngToClipboard(node: HTMLElement): Promise<{ ok: true } | { ok: false; error: string }> {
...
function StaffDetailPanel({
  ...
  const deletePayrollBang = useCallback(
    async (bl: AdminBangTinhLuongListItem) => {
      if (!canEdit) return;
      const kyLabel =
        bl.ky_thang || bl.ky_nam ? [bl.ky_thang, bl.ky_nam].filter(Boolean).join(" ") : "—";
      const confirmed = window.confirm(
        `Bạn sắp xóa bảng lương #${bl.id} (${kyLabel}).\n\n` +
          `Mọi lịch điểm danh gắn bảng này sẽ bị xóa theo. Thao tác không thể hoàn tác.\n\n` +
                              ...
                              >
                                ...
                                <td className="w-12 px-1 py-1 text-right align-middle" onClick={(e) => e.stopPropagation()}>
                                  <button
                                    ...
                                    onClick={() => void deletePayrollBang(bl)}
