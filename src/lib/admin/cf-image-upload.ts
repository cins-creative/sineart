...
export type CfWorkerPostResult =
  | { ok: true; url: string }
  | { ok: false; error: string };

/** POST multipart (field `file`) → Worker upload-cf-images — dùng chung single upload & batch. */
export async function postFormDataToCfWorker(form: FormData): Promise<CfWorkerPostResult> {
  const secret =
  ...
  if (!secret) {
    return { ok: false, error: "Chưa cấu hình SINE_ART_WORKER_SECRET trên server." };
  }
  ...
  try {
    ...
    const agentBundleOnUploadHost = wrongAgentWorker && /sine-art-api/i.test(host);
