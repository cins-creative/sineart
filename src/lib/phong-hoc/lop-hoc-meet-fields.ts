...
/** Row thô từ PostgREST — hai query select khác cột nên gộp kiểu thủ công. */
type QlLopMeetFetchRow = {
...
export async function fetchLopHocMeetRow(
  ...
  let data: QlLopMeetFetchRow | null = first.data as QlLopMeetFetchRow | null;
  ...
  if (missingSetAtCol) {
    const retry = await supabase
      ...
      .eq("id", lopHocId)
      .maybeSingle();
    data = retry.data as QlLopMeetFetchRow | null;
    error = retry.error;
  }
