...
const CATALOG_URL = `${SITE_ORIGIN}/tra-cuu-thong-tin` as const;
...
/** Breadcrumb listing — mục 2: «Thông tin đại học» (theo brief SEO). */
...
/**
 * `/tra-cuu-thong-tin/[slug]` — `Article` (một script JSON-LD).
 */
...
/** `Dataset` — chỉ bài loại điểm chuẩn (`diem-chuan`). */
